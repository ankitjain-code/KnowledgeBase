...
class CBXWPBookmark_Public {
	...
	public function bookmark_mycat_shortcode( $attr ) {
		...
		else if(is_string($userid_temp)){
			$user_temp = get_user_by( 'login', $userid_temp );
			if($user_temp !== false){
				$userid_temp = absint($user_temp->ID);
				if($userid_temp > 0){
					$attr['userid'] = $userid_temp;
				...
				//user not found by this username so reset to guest
				$attr['userid'] = 0;
			}
		}
	...
	public function enqueue_scripts() {
		...
		$cbxwpbookmark_translation = array(
			...
			'category_default_status'            => $category_default_status, //default category status if user category mode is enabled
		);
